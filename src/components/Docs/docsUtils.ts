...
export function createDocsTree(
  docs: DocType[],
  rootLevel: 'seller' | 'docDate' = 'seller',
) {
  let roots: { rootId: string | number; children: any[] }[] = [];
  let docsTree: TreeNode[] = [];

  function getCondition(d: DocType, rootId?: string | number | null) {
  ...
  docs.forEach((d) => {
    let root = roots.find((r) => getCondition(d, r.rootId));
    // add root level
    if (!root) {
      root = {
        rootId:
...
export function getFilterDocs(
  rootLevel: 'seller' | 'docDate',
  d: DocType,
  docsPid: string,
) {
  if (rootLevel === 'seller') {
